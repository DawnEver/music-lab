...
- In `/play` the instrument is the only choice: its `surface` decides what is drawn (keys, frets, pads, holes) and its `timbre` decides what is heard. A separate voice menu would let the name and the sound disagree. How it is *sounded* — model, recording, or the recording's attack over the model — is a different question with the same answer for all three; it sits on the bar beside the direction the instrument runs, not behind the setup sheet, because all three play the instrument the picker names.
...
- Layout follows use: the tuning workbench shows many readouts at once; the other tools each have one focus that never scrolls — **at every viewport, not just the one it was designed on**. Smoke asserts it for each of them.
- **Chrome is space taken from the tool.** Brand, nav, toggles, colophon and padding cost 41% of a 667px phone before anything is played, so on a phone the toggles ride beside the brand, the nav drops its icons, the colophon is one line and the decorative kicker goes. When a tool still will not fit, take the room from the chrome before taking it from the instrument or the picture.
...
- **A key has a size, and both ends of it come from the hand.** A white key is never wider than a real one (23.5mm) and never narrower than a fingertip (9mm); between those the keyboard fills the room, above it centres, below it scrolls. The player may set any width *between* them — the room decides until they do, and stops deciding once they have — because a floor and a ceiling are still a choice between two widths and a hand is not the same hand as the room. Bounds stated in physical units say why they are those numbers, and they live in `play/domain/layout.ts` because the slider, the store and the board's clamp all have to agree.
...
- **A timbre is measured, never remembered.** `npm run timbre` renders every tier through the app's own engine and reports the numbers a listener names when they say a sound is wrong — level, attack, ring, brightness, harmonics — plus WAVs to A/B by ear, and a second run reads the first as a baseline. An ear is still the only instrument that can say whether a difference matters, but "which of three tiers is louder, and by how much" is not a question for memory.
