...
- Chrome belongs to whoever uses it, not to the shell: `SourceBar` is a shared component placed by each tool that needs an input (tuning, trace, sight-singing) — it never moves into the shell, and it never appears in a tool that has no use for a microphone.
